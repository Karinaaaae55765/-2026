...
$specPath = Join-Path $q4Dir "Q4_Robot_v5.spec"
$distPath = Join-Path $projectRoot "dist"
$buildRoot = Join-Path $projectRoot "build"
$workPath = Join-Path $buildRoot "q4_exe_v5"
$exePath = Join-Path $distPath "Q4_Robot_v5.exe"
$simulatorPath = Join-Path $projectRoot "Jammers-simulator-win64\Jammers-simulator\Q4_Robot_v5.exe"
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $exePath -PathType Leaf)) { throw "Q4 V5打包失败" }
& $exePath --self-check
if ($LASTEXITCODE -ne 0) { throw "Q4 V5离线自检失败" }
